...
class HashContext():
    ...
    def decode_token(self, token: str, key:str, algorithm:str):
        if self.is_token_in_deny_list(token): 
            return {}
        
        try:
            decoded_token = jwt.decode(token, key, algorithm)
    ...
    def decode_access_token(self, token: str) -> dict:
        return self.decode_token(token, self.JWT_SECRET_KEY, self.ALGORITHM)
